...
# Diseases that are ONLY available in sentinel surveillance (teiten)
# These will be included from teiten data; all others come from zensu/confirmed
SENTINEL_ONLY_DISEASES = {
    "RSV",
...
def get_sentinel_only_diseases() -> set[str]:
    """Get the set of diseases that are only available in sentinel surveillance.

    Returns:
        Set of disease names that should only come from teiten data.
    """
    return SENTINEL_ONLY_DISEASES.copy()
...
def smart_merge(
    ...
    - Add ONLY sentinel-exclusive diseases from teiten data
    ...
    """
    sentinel_only = get_sentinel_only_diseases()

    # Filter teiten to only include sentinel-exclusive diseases
    teiten_filtered = teiten_df.filter(pl.col("disease").is_in(list(sentinel_only)))
